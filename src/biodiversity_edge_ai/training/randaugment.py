...
'''
# Adapted for package-local use: removed global absl flags; the trainer controls
# TensorFlow's random seed. See licenses/RandAugment-LICENSE.txt.
import inspect
import math
...
# Randomness is controlled by tf.keras.utils.set_random_seed in the trainer.
...
def distort_image_with_randaugment(image, num_layers, magnitude,
  ...
  for layer_num in range(num_layers):
    op_to_select = tf.random.uniform(
        [], maxval=len(available_ops), dtype=tf.int32, seed=None)
    random_magnitude = float(magnitude)
    with tf.name_scope('randaug_layer_{}'.format(layer_num)):
      for (i, op_name) in enumerate(available_ops):
        prob = tf.random.uniform(
          [], minval=0.2, maxval=0.8, dtype=tf.float32, seed=None)
